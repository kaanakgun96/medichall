...
describe("External Prospect Discovery production contract", () => {
  it("uses one shared workspace across first-class Buyer Discovery surfaces", () => {
    for (const page of [portal, standalone]) {
      expect(page).toContain("external-prospects.js?v=20260826adminfresh1");
      expect(page).toContain("external-prospects.css?v=20260826adminfresh1");
    }
  ...
  it("keeps discovery manual, bounded, cached, and zero-paid-provider", () => {
    ...
    expect(externalWorkspace).toContain('if(action==="discover")discover("NORMAL_DISCOVERY")');
    expect(externalWorkspace).toContain('else if(action==="retry")discover("NORMAL_DISCOVERY",true)');
