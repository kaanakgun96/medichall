...
export function normalizeActivitySignal(input: {
  ...
  let normalizedClass: ActivitySignal["normalizedClass"] = "NON_MATCH";
  let strength: ActivitySignal["strength"] = "NON_MATCH";
  if (
    ...
    /46\.69|46\.90|medical|surgical|hospital|healthcare/.test(searchable) &&
  ...
  return {
    ...
    registryIdentifier: input.registryIdentifier
      ? sanitizeEvidenceText(input.registryIdentifier, 240)
      : null,
    nationalCode: sanitizeEvidenceText(input.nationalCode, 40),
    nationalClassification: sanitizeEvidenceText(
      input.nationalClassification,
      80,
    ),
    description,
    normalizedNaceCode: /^\d{2}(?:\.\d{1,2})?$/.test(code) ? code : null,
    naceRevision: input.naceRevision || "NATIONAL_ONLY",
    normalizedClass,
    strength,
