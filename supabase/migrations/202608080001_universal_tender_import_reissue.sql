-- MedicHall Universal Tender Import production reissue
--
-- Forward-only atomic port of the immutable 202607290001 base and 202607290002
-- final hardening sources after production migration 202608020001. The archived
-- history remains unchanged and intentionally absent from the production ledger.
-- Both source contracts execute in one transaction so no unhardened intermediate
-- table, policy, grant, or function can become visible if hardening fails.
...
drop policy if exists "owners read own imported tenders"
  on public.tenders;
-- The canonical chain defines authenticated tender RLS policies but older
-- production history also supplied the underlying SELECT grant out of band.
-- Reconcile that privilege forward-only so a fresh install can actually use
-- both the existing open-tender policy and the private-import owner policy.
...
-- As with tenders, the fresh-install baseline has the RLS policy but no base
-- SELECT privilege; production received that grant from historical runtime
-- setup. Keep the policy effective without granting mutation rights.
grant select on table public.tender_documents to authenticated;
...
-- Final blocker remediation and security hardening is part of the same atomic
