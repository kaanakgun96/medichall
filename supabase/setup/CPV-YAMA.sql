-- ============================================================
-- CPV-YAMA.sql — Akıllı CPV eşleştirme
--
-- Sorun 1: "33190000-8" (resmî yazım, kontrol basamaklı) girilince,
--   ihalelerde kontrol basamaksız saklanan "33190000" ile birebir
--   karşılaştırma TUTMUYOR, CPV skoru her ihalede sıfır çıkıyordu.
-- Sorun 2: CPV hiyerarşiktir — 33190000 bir aile kodudur, 33192000
--   gibi alt kodlarını da kapsamalıdır.
--
-- Çözüm: cpv_overlap_score()
--   • Rakamları ayıklayıp İLK 8 HANEYİ alır ("33190000-8" → "33190000";
--     kontrol basamağı da rakam olduğu için bu şart)
--   • Birebir eşleşme → puan
--   • Hiyerarşik eşleşme → aile kodunun sondaki sıfırları atılıp
--     kök olarak kullanılır (33190000 → "3319" → 33192000 ✓, 33140000 ✗)
-- Tekrar çalıştırmaya dayanıklıdır.
-- ============================================================

create or replace function public.cpv_overlap_score(
...
as $cpv$
  ...
  matched as (
    select p.code
      from profile_codes p
     where exists (
        select 1
          from opportunity_codes o
         where o.code = p.code
            or (
              length(rtrim(p.code, '0')) between 2 and 7
              and o.code like rtrim(p.code, '0') || '%'
            )
      )
  ),
  totals as (
    select
      (select count(*) from profile_codes) as profile_count,
      (select count(*) from matched) as match_count
  )
  select case
    when profile_count = 0 then 0
    else least(100, round(100.0 * match_count / profile_count))::integer
  end
  from totals;
$cpv$;

-- ============================================================
-- refresh_company_opportunity_matches: CPV çağrısı yeni fonksiyona
-- yönlendirilmiş tam sürüm (v2 skorlamanın birebir aynısı, tek fark
-- array_overlap_score → cpv_overlap_score)
-- ============================================================
...
begin
  ...
  cross join lateral (
    select
      ...
      public.cpv_overlap_score(v_profile.cpv_codes, t.cpv_codes) as cpvs,
...
select 'cpv yamasi tamam' as sonuc;
