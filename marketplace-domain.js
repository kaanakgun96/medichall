...
(function (global) {
  ...
  function productSearchText(product) {
    return lower([
      product.name, product.ref, product.category, product.normalized_category,
  ...
  function similarProduct(source, candidate) {
    ...
    let score = 0;
    const reasons = [];
    if (source.category && source.category === candidate.category) { score += 28; reasons.push(`Same category: ${source.category}`); }
  ...
  const comparisonRows = [
    ...
    ["Category", (product) => product.category], ["Subtype", (product) => product.product_subtype],
