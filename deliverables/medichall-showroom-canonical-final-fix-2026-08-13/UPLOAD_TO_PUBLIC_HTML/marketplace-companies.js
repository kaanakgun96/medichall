...
(function (global) {
  ...
  function companyCategories(company) {
    return values(productsFor(company.id).map((product) => product.taxonomy_category || product.category));
  }
  ...
  function renderTools() {
    ...
    if (!host) return;
    const categories = values(state.products.map((product) => product.taxonomy_category || product.category));
  ...
  async function initDirectory() {
    try {
      ...
      if (productIds.length) {
        try {
          mappings = await request(`product_taxonomy_mappings?select=product_id,medical_product_taxonomy(canonical_name)&is_primary=eq.true&status=eq.approved&product_id=in.(${productIds.join(",")})`);
        } catch (_) { /* Use legacy public categories until taxonomy is available. */ }
      }
      const taxonomyByProduct = new Map((mappings || []).map((mapping) => {
        ...
        return [Number(mapping.product_id), taxonomy?.canonical_name || null];
      }));
      state.products = (products || []).map((product) => ({ ...product, taxonomy_category: taxonomyByProduct.get(Number(product.id)) || null }));
      renderTools(); renderDirectory();
    } catch (_) {
      const grid = document.getElementById("dirGrid");
  ...
  function setProfileMetadata(company, products) {
    const slug = String(company.slug || "").trim();
    const canonicalUrl = slug
      ? `https://medichall.com/m/${encodeURIComponent(slug)}`
      : `https://medichall.com/companies`;
    const description = String(company.description || "").trim() ||
      `${company.name} is listed in the MedicHall medical-industry company directory as ${company.type || "a company"}${company.country ? ` in ${company.country}` : ""}.`;
    const compactDescription = description.replace(/\s+/g, " ").slice(0, 200);
    const image = D?.safeHttpUrl(company.logo_url) || "https://medichall.com/og-cover.png";
    const companyWebsite = company.website
      ? D?.safeHttpUrl(/^https?:\/\//i.test(company.website) ? company.website : `https://${company.website}`)
      : null;
    const title = `${company.name} — MedicHall Company Showroom`;
    document.title = title;
    document.querySelector('meta[name="description"]')?.setAttribute("content", compactDescription);
    document.getElementById("mhCanonical")?.setAttribute("href", canonicalUrl);
    document.getElementById("mhOgTitle")?.setAttribute("content", title);
    document.getElementById("mhOgDescription")?.setAttribute("content", compactDescription);
    document.getElementById("mhOgUrl")?.setAttribute("content", canonicalUrl);
    document.getElementById("mhOgImage")?.setAttribute("content", image);
    document.getElementById("mhTwitterTitle")?.setAttribute("content", title);
    document.getElementById("mhTwitterDescription")?.setAttribute("content", compactDescription);
    document.getElementById("mhTwitterImage")?.setAttribute("content", image);
    document.getElementById("mhDirectoryStructuredData")?.setAttribute("type", "application/json");

    const categories = values((products || []).map((product) => product.taxonomy_category || product.category));
    const organization = {
      ...
      ...(image !== "https://medichall.com/og-cover.png" ? { logo: image } : {}),
      ...
      ...(categories.length ? { knowsAbout: categories.slice(0, 20) } : {}),
      ...(companyWebsite ? { sameAs: [companyWebsite] } : {}),
      mainEntityOfPage: canonicalUrl,
    };
    const script = document.getElementById("mhCompanyStructuredData");
    if (script) script.textContent = JSON.stringify(organization);
  }

  async function enhanceProfile(company, products, certificates = []) {
    if (!company) return;
    setProfileMetadata(company, products);
  ...
  global.MedicHallEnterpriseCompanies = {
    initDirectory,
    enhanceProfile,
    applyProfileMetadata: setProfileMetadata,
  };
})(globalThis);
