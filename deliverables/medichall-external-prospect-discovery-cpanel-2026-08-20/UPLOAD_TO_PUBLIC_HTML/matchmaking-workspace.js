...
const edgeRequest=(name,body)=>request("/functions/v1/"+name,{method:"POST",body:JSON.stringify(body)});
...
function profileForm(profile){
  ...
  return '<section class="card pad"><div class="panel-head"><div><h2>'+(profile?"Your two-sided match profile":"Set up partner matchmaking")+'</h2><p class="sub">This structured profile powers the recommendations and explanations in this dedicated workspace.</p></div></div>'+
    ...
    '<div class="grid2"><div id="taxonomyOffered"></div><div id="taxonomyInterested"></div></div>'+
    '<details><summary>Legacy product phrases</summary><p class="sub">Preserved for auditability and fallback matching.</p><div class="grid2"><div><label for="pOffered">Legacy products offered</label><textarea id="pOffered" rows="3">'+esc(value("offered_products"))+'</textarea></div><div><label for="pInterested">Legacy products sought</label><textarea id="pInterested" rows="3">'+esc(value("interested_products"))+'</textarea></div><div><label for="pCategories">Legacy product categories</label><input id="pCategories" value="'+esc(value("product_categories"))+'"></div></div></details>'+
    '<div class="grid2"><div><label for="pPartners">Partner types sought</label><input id="pPartners" value="'+esc(value("partner_types_sought")||(COMPANY?"distributor, buyer":"manufacturer, distributor"))+'"></div>'+
...
function initTaxonomySelectors(){
  state.taxonomySelectors.forEach(selector=>selector.destroy());state.taxonomySelectors=[];
  const create=(root,kind,label,hint)=>{
    if(!document.querySelector(root))return;
    state.taxonomySelectors.push(globalThis.MedicHallTaxonomy.createSelector({root,interestKind:kind,label,hint,
      placeholder:"Search medical products…",search:taxonomySearch,resolve:taxonomyResolve,
      initial:utils.array(state.taxonomyInterests).filter(item=>item.interest_kind===kind)}));
  };
  create("#taxonomyOffered","offered","Products your company offers","Choose categories represented by your commercial products");
  create("#taxonomyInterested","interested","Products you are interested in","Select a family or specific category");
}

function profileCompleteness(payload,taxonomyCount=0){
  ...
  if(taxonomyCount>0)values[3]=taxonomyCount;
...
async function saveProfile(){
  ...
  const creatingMatchmakingProfile=!state.data?.profile;
  const taxonomyInterests=state.taxonomySelectors.flatMap(selector=>selector.getValue());
  payload.profile_completeness=profileCompleteness(payload,taxonomyInterests.length);setBusy(button,true,"Saving…");
  try{
    const saved=await db("matchmaking_profiles?on_conflict=user_id",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=representation"},body:JSON.stringify(payload)});
    const profileId=saved?.[0]?.id||state.data?.profile?.id;
    if(!profileId)throw new Error("Saved matchmaking profile identifier is unavailable");
    await rpc("save_matchmaking_taxonomy_interests_v1",{p_profile_id:profileId,p_interests:taxonomyInterests});
    state.view="matches";await loadWorkspace(true);toast("Match profile saved ✓");
    if(creatingMatchmakingProfile) globalThis.MedicHallTraffic?.trackConversion?.("matchmaking_profile_created",{once:true});
  }catch(error){toast(parseError(error));}
  finally{setBusy(button,false);}
}

function matchesView(){
  globalThis.MedicHallTraffic?.trackConversion?.("match_viewed",{once:true});
...
function matchCard(match){
  ...
  const components=utils.array(explanation.components);
  ...
  return '<article class="match-card"><div class="match-top"><div class="company-ident">'+logo(target)+'<div><div class="match-role">'+esc(target.role||"partner")+'</div><div class="match-title">'+esc(target.display_name||"Unnamed company")+(target.verified?'<span class="verified"> · Verified</span>':"")+'</div><div class="match-meta">'+esc(target.country||"Country not specified")+(target.company_size?" · "+esc(target.company_size):"")+' · <span class="status '+esc(status)+'">'+esc(utils.statusLabel(status))+'</span></div></div></div><div class="score" style="--score:'+Number(match.match_score||0)+'"><b>'+Number(match.match_score||0)+'%</b></div></div>'+
    '<p class="sub" style="margin-top:12px">'+esc(explanation.summary||"Review the structured compatibility evidence below.")+'</p>'+globalThis.MedicHallTaxonomy.productEvidenceHtml(explanation.product_evidence)+
    (drivers.length?'<div class="driver-grid">'+drivers.slice(0,3).map(driver=>'<div class="driver supported"><b><span>'+esc(driver.label)+'</span><span>'+Number(driver.score||0)+'%</span></b><p>'+esc(driver.reason)+' · '+Number(driver.weight_percent||0)+'% weight</p></div>').join("")+'</div>':'<div class="evidence-note">No supported positive drivers yet. This score is directional until both companies add explicit structured profile evidence.</div>')+
    (components.length?'<details><summary class="sub">Review every score component</summary><div class="driver-grid">'+components.map(component=>'<div class="driver '+esc(component.evidence_status||"unknown")+'"><b><span>'+esc(component.label||"Evidence component")+'</span><span>'+(component.evidence_status==="unknown"?"Unknown":Number(component.score||0)+"%")+'</span></b><p>'+esc(component.reason||"No explicit evidence is available.")+' · '+Number(component.weight_percent||0)+'% weight</p></div>').join("")+'</div></details>':"")+
...
async function sendConnection(targetId){
  ...
  try{
    ...
    globalThis.MedicHallTraffic?.trackConversion?.("connection_requested");
...
async function sendMeeting(saveDraft){
  ...
  try{
    ...
    if(!saveDraft&&!['counter','reschedule','edit_proposal','edit_draft'].includes(mode)) globalThis.MedicHallTraffic?.trackConversion?.("meeting_scheduled");
