...
(function (global) {
  ...
  let lastPageRequest = Promise.resolve();
  const oncePerPage = new Set();
  ...
  function sessionNeedsPageView(now) {
    let session = memorySession;
    if (!session) {
      try { session = JSON.parse(storageRead(SESSION_KEY) || "null"); } catch (_) { session = null; }
    }
    return !session || !UUID_PATTERN.test(String(session.id || "")) ||
      !Number.isFinite(Number(session.last_activity_at)) ||
      now - Number(session.last_activity_at) > SESSION_TIMEOUT_MS ||
      now < Number(session.last_activity_at);
  }
  ...
  function normalizedRoute() {
    ...
    if (file === "medical-device-tenders.html" || file === "medical-device-tenders") return "tenders";
    if (file === "ai-tender-intelligence.html" || file === "ai-tender-intelligence") return "tenders";
    if (file === "find-medical-device-distributors.html" || file === "find-medical-device-distributors") return "companies";
    if (file === "medical-device-b2b-marketplace.html" || file === "medical-device-b2b-marketplace") return "products";
    if (file === "ai-medical-device-matchmaking.html" || file === "ai-medical-device-matchmaking") return "matchmaking";
  ...
  function track(route, options) {
    ...
    lastPageRequest = global.fetch(ENDPOINT, {
      method: "POST",
      headers: {
  ...
  function trackConversion(eventType, options) {
    if (!trackingPreferenceAllows() || !CONVERSION_EVENTS.has(String(eventType || ""))) return false;
    const once = options?.once === true;
    if (once && oncePerPage.has(eventType)) return false;
    if (once) oncePerPage.add(eventType);
    const now = Date.now();
    if (sessionNeedsPageView(now)) track(null, { force: true });
    const session = currentSession(now);
    const payload = {
      event_id: uuid(),
      visitor_id: visitorId(),
    ...
    lastPageRequest.then(function () {
      ...
      // Conversions are best-effort: no retry and no product/auth impact.
    });
    return true;
  }
  ...
  function resetForTest() {
    ...
    lastRoute = null;
    lastRecordedAt = 0;
    lastPageRequest = Promise.resolve();
    oncePerPage.clear();
  }
