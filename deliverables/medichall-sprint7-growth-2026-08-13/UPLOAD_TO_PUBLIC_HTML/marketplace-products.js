...
(function (global) {
  ...
  const state = {
    ...
    recommendationResults: [], selectedProduct: null, detailReturnFocus: null, rfqReviewed: false,
  };
  ...
  async function loadProducts() {
    ...
    if (productIds.length) {
      try {
        mappings = await request(`/rest/v1/product_taxonomy_mappings?select=product_id,taxonomy_id,medical_product_taxonomy(canonical_name,slug,node_type)&is_primary=eq.true&status=eq.approved&product_id=in.(${productIds.join(",")})`);
      } catch (_) { /* Taxonomy is a progressive enhancement until the migration is deployed. */ }
    }
    const byProduct = new Map((mappings || []).map((mapping) => {
      const taxonomy = Array.isArray(mapping.medical_product_taxonomy)
        ? mapping.medical_product_taxonomy[0] : mapping.medical_product_taxonomy;
      return [Number(mapping.product_id), {
        taxonomy_id: Number(mapping.taxonomy_id),
        taxonomy_category: taxonomy?.canonical_name || null,
        taxonomy_slug: taxonomy?.slug || null,
      }];
    }));
    state.products = rows.map((row) => D.normalizeProduct({ ...row, ...(byProduct.get(Number(row.id)) || {}) }));
  }
  ...
  function card(product) {
    ...
    return `<article class="marketplace-card" data-product-ref="${escapeHtml(product.ref)}">
      <div class="marketplace-card__image">
        ...
        <div class="marketplace-card__eyebrow">${escapeHtml(product.product_subtype || product.taxonomy_category || product.category)}</div>
  ...
  function renderDetail(product, updateUrl = true) {
    ...
    const previousRef = state.selectedProduct?.ref;
    const previousScrollTop = drawer.querySelector(".marketplace-detail__scroll")?.scrollTop || 0;
    ...
    drawer.innerHTML = `<button class="drawer-close" type="button" data-detail-action="close" aria-label="Close product details">×</button>
      <div class="marketplace-detail__scroll" tabindex="0" aria-label="Product details content">
      ...
      <div class="marketplace-detail__summary"><div class="marketplace-breadcrumbs">Products / ${escapeHtml(product.taxonomy_category || product.category)} / ${escapeHtml(product.ref)}</div><div class="marketplace-card__eyebrow">${escapeHtml(product.product_subtype || product.taxonomy_category || product.category)}</div><h2 id="dTitle">${escapeHtml(product.name)}</h2>
      ...
      <div class="marketplace-detail__content"><section class="marketplace-detail-section"><h3>Technical and commercial profile</h3><dl class="marketplace-specs">
        ${spec(product, "Category", "normalized_category", product.product_subtype || product.taxonomy_category || product.normalized_category || product.category)}
      ...
      <section class="marketplace-detail-section" style="margin-top:10px"><h3>Similar products</h3><div class="marketplace-similar">${similar.length ? similar.map((item) => `<button type="button" data-similar-ref="${escapeHtml(item.product.ref)}"><b>${escapeHtml(item.product.name)}</b><small>${escapeHtml(item.reasons.join(" · "))}</small></button>`).join("") : `<p class="marketplace-unknown">Insufficient structured evidence for useful alternatives.</p>`}</div></section></aside></div></div>`;
    drawer.classList.add("open");
    document.getElementById("drawerBackdrop").classList.add("open");
    document.body.style.overflow = "hidden";
    if (previousRef === product.ref) drawer.querySelector(".marketplace-detail__scroll").scrollTop = previousScrollTop;
  ...
  async function recipientIds(product) {
    const multi = state.user && document.getElementById("rfq-multi").checked && (product.taxonomy_id || product.category);
    if (!multi) return product.company_id ? [product.company_id] : [];
    if (product.taxonomy_id) {
      const mappings = await request(`/rest/v1/product_taxonomy_mappings?select=products(company_id,is_active)&taxonomy_id=eq.${encodeURIComponent(product.taxonomy_id)}&is_primary=eq.true&status=eq.approved&limit=250`);
      return [...new Set((mappings || []).flatMap((row) => {
        const products = Array.isArray(row.products) ? row.products : [row.products];
        return products.filter((item) => item?.is_active).map((item) => Number(item.company_id)).filter(Boolean);
      }))];
    }
  ...
  async function updateRecipientSummary() {
    ...
    try {
      ...
      target.textContent = `This category request will be sent once to each of ${ids.length} eligible companies with an active ${product.taxonomy_category || product.category} product.`;
  ...
  async function submitRfq() {
    ...
    try {
      ...
      global.MedicHallTraffic?.trackConversion?.("rfq_created");
  ...
  function bindEvents() {
    ...
    document.getElementById("drawer").addEventListener("click", (event) => {
      ...
      if (action === "rfq" && state.selectedProduct) { const product = state.selectedProduct; closeDetail(false); openRfq(product); }
    ...
    document.addEventListener("keydown", (event) => {
      ...
      const focusable = [...document.getElementById("drawer").querySelectorAll("a[href],button:not([disabled]),input,select,textarea,[tabindex=\"0\"]")].filter((item) => item.offsetParent !== null);
